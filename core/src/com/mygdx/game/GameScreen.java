...
public class GameScreen extends ScreenAdapter{
	...
	public void render(float delta) {
		...
		if (ballX < 0 || ballX + ballWidth > Gdx.graphics.getWidth()) {
			ballXSpeed *= -1;
			ping.play(1);
		}
		else if (ballY < 0 || ballY + ballWidth > Gdx.graphics.getHeight()) {
			ballYSpeed *= -1;
			ping.play(1);
		}
		
		if (paddleBox.overlaps(ballBox)) {
			ballXSpeed *= -1;
			score++;
			ping.play(1);
		}
		...
		if (ballBox.getX() == 0) {
			...
			batch.begin();
			font.draw(batch, "Tap The Screen to Begin", Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight() / 2);
			batch.end();
	...
	private class paddleControl extends InputAdapter {
		...
		public boolean touchDown(int screenX, int screenY, int pointer, int button) {
			/*if (button == Input.Buttons.LEFT && paddleY + 95 < Gdx.graphics.getHeight()) {
				paddleY += paddleSpeed;
				paddleBox.setY(paddleBox.getY() + paddleSpeed);
			}
			else if (button == Input.Buttons.up && paddleY
					> 0) {
				paddleY -= paddleSpeed;
				paddleBox.setY(paddleBox.getY() - paddleSpeed);
			}
			else*/ if (button == Input.Buttons.LEFT && readyForSpace && lives > 0) {
